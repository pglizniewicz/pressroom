...
MARKUP (Drupal 7, views + facetapi)
  ...
  detail_id `<article id="node-4935591">` -> "4935591". Drupal's own node id,
            ...
            (serve.wayback_url, db._TS_GLOB), and faking one here would put a
  ...
  date      TWO formats, both in the listing, and picking the wrong parser
            ...
            2000-03-21. Same trap as scrape_terratec_portal.py.
...
Usage:
  python scrape_soundonsound.py --limit 3      # dry run, 3 articles
  python scrape_soundonsound.py --list-only    # walk the listings, store nothing
  python scrape_soundonsound.py
"""
...
def parse_listing(content: bytes) -> list:
...
def fetch_body(conn, session: requests.Session, url: str) -> tuple:
    ...
    missing. Signature fixed by backfill_body_html.LIVE_FETCHERS."""
...
def collect(conn, session, pages: int = None) -> dict:

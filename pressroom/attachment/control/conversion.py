...
def plain_text(content: bytes) -> tuple[str, str]:
...
def rotated_text(content: bytes) -> list[str]:
...
def _page_lines(page) -> list[tuple[float, float, list[str]]]:
...
def _visual_lines(lines: list) -> list[tuple[float, float, float, str, float]]:
...
def to_richtext(content: bytes) -> tuple[str, str, str]:

...
So: one module, one concern - turn a parsed node into
    ...
    body        plain text with real paragraphs, for FTS and search.py
...
Imports bs4, so this module must never be imported by db.py, search.py or
serve.py - those three stay dependency-free (CLAUDE.md invariant 4).
...
arrangement as encoding.C1_RE vs db._MOJIBAKE_SQL: two languages, one rule.
...
def to_text(html: str) -> str:
    ...
    the FTS index and search.py's snippets see.

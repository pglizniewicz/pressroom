...
- **`.venv/bin/python -m unittest discover -s tests` first, after anything.**
  ...
  **`tests/refresh.py --golden` after an intentional parser change**, then read
  ...
  produced** — `gate.py`'s allowances, the cp1258 mojibake, the 122 articles
  overwritten by listing teasers — and the docstrings carry that record, same
  rule as everywhere else here. And **a test that cannot fail is worse than no
  test**; `'integrity-check'` passing on a corrupt index is this repo's own
...
- **`uvx ruff format .` and `uvx ruff check .` before a commit.** The tree went
  ...
  - **`E501` is out.** After formatting, the lines still over 88 are HTML
    ...
    things a formatter will split or should. Enabling it adds a `noqa` per line
    and nothing else.
  - **`docstring-code-format` stays off.** This repo's docstrings carry markup
    samples, and they are not code to be reformatted.
  ...
  The adoption itself required a proof rather than an assurance: a digest over
  ...
  every move leaves the same two problems: a
  name that moved without its import, and an import that stayed after its use
  left. See the `F821` case in the next entry for what the first one cost.
...
- **`pressroom-verify-names` after any change that renames or moves a
  ...
  the restructuring: *imports* (every module imports — it executes module level
  only, and nothing below it) and *a local shadowing an imported module* (a line
  assigning to a local `gate` shadows the module for the whole function, so the
  name *is* bound, just too late).
  ...
  `foo.bar` and missed a bare `foo(...)`, which is an `ast.Call` on
  a plain `ast.Name`. `fetcher/control/archive.py`
  called `snapshot_url()` in three places and imported the module it is in
  ...
  What it cannot check — it is static, and a scraper's phase 1 has to be *run* —
  is `tests/test_offline_is_offline.py`, which invokes all 15 scrapers under
  ...
  The other thing it cannot check is prose: it resolves Python names, and a docstring that
...
- **The PyCharm inspection report, through the MCP, read against `HEAD`.** The
  ...
  **Read it as a diff, not as a pass or fail.** This tree's report is never empty and never
  ...
  directory still present at the next `git add -A` is in the next commit.

  **The baseline run doubles as proof the inspection still reports**, which it
  needs, because a file with no findings gets no entry at all: "clean" and
  ...
  **`Duplicated code fragment` is what justifies the round trip.** Ruff
  ...
  copies removed from this tree were both invisible to it and both had passed a
  ...
  copy is a pair, and the other half is in a file the change did not touch.
  ...
  **Not headless `bin/inspect.sh`, though it is installed.** It requires an
  ...
  into a tree that excludes it, and starting a second IDE to re-index a
...
- **`.venv/bin/python`, not bare `python3`.** `pressroom` is only on the venv's
  path, so bare `python3` cannot import it — which fails visibly, and used to
  fail silently: everything ran on Fedora's system RPMs until the first
  dependency Fedora does not package.
- `pressroom.db` is gitignored, along with `pressroom.db.bak`. `'rebuild'` and
  ...
  `--force` gets tried against that copy, without moving the file in and
  out of place.
...
- Not wanted here: an ORM, a query builder, a row dataclass, schema churn on
  ...
  exception: `grade`, because the alternative was leaving a grade

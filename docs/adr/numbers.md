...
standing decisions that survived it. The section used to live in `CLAUDE.md`;
the rule holds for these files just as hard.

**Not here.** `CLAUDE.md` used to carry a 259-line "Known open state" section of
corpus tallies and dated run logs, and it rotted exactly the way that kind of
prose does: three different figures for the same metric, a claim and its own
retraction a hundred lines apart, and an explanation that stayed after the
number it explained had moved. The counts are now asserted where they can fail:

- **corpus counts and index health** — `tests/corpus/`, which asserts invariants
...
`CLAUDE.md` or into one of these files, that is the signal.
...
- **A `media_news` teaser is the expected end state.** Two thirds of those rows
  ...
  rest found genuine absence rather than a bad day. An hour of crawl once bought
  one row. **Measure the URL scheme before spending a crawl on a block like that.**
...
- **The rows whose bytes exist only under a mirror domain have no capture of
  ...
  and it is now knowledge — do not re-buy it.

...
at all. One more detail: a prefix matches as a string, not as a path segment, so
the listing for `/presse/pressemit/` also carries `/presse/pressemit.htm` — the
index page itself, which the html filter accepts and which would otherwise be
stored as a release.
...
served the same release *files*, backed per row rather than assumed: identical
...
**Duplication across tags is intended.** The same release is on several
...
teaser, across tags it would record something false.
...
is what tells them apart.** What two tags provide is the refusal to pair across
them. Across mirrors that refusal is needed: the same release on midiman.net
and midiman.com carries the same title and the same date, so one tag there
would put real duplicates in one `twin.fill` group. Across languages it has no
effect, because the title differs for the reason the tag existed — the
language. So **merging the ten domain-only tags stays undone** (it would also
move the panel, the audit, `taxonomy/entity/company.py` and five `checks.md`
fixtures), and the language pairs are a different decision, taken once.
...
- **Would `twin.fill` start pairing?** Two (collapsed title, exact date) groups
  ...
  `pressemit` passes no `twins_too` anyway. Possible, not actual.
...
- **What does the dedup key do when the tag stops being one host?** This is the
  one that caused a problem: **58 of the generation's 166 filenames exist on
  both hosts**, because the German copy of a release keeps the release's
  filename. A check keyed on the filename alone — which is what it was when each
  host had its own tag — reports all 58 German copies already-stored, silently.
  The key took the site as its first half (`pressemit.site_of`, partitioning by
  url path), and `tests/terratec/test_pressemit.py` is what holds it there.
...
an explicit table, never a prefix rule — an unmapped source is put in a visible
`inne` bucket rather than left out of the counts (`taxonomy/entity/company.py`).

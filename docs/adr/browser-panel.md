...
an undated source sorts last, since `""` sorts before 1996.
...
modifier click is intercepted, which knowingly gives up open-in-new-tab there.

**The panel is its own scroll container from 60rem up, and `sticky` alone was not
enough** — a sticky element taller than the viewport scrolls with the page and
...
`padding-inline-end` (accounted for by `calc(15rem + var(--gap))`) exists
because an overlay scrollbar paints *on top of* content and
`scrollbar-gutter: stable` reserves nothing for one; taking the strip out of
the 15rem instead wrapped the three longest tags to a third line. Both the
sticky offset and the max height are `100dvh` minus **`--topbar-h`, measured in
`app.js` with a `ResizeObserver`** rather than hardcoded, because the filter
form wraps between 60rem and ~72rem.

**A connection lasts one request, and the thread-local it replaced was never a
cache.** `do_GET` used to keep a `connect_ro()` handle on a
...
and SQLite enforces the rule where a comment only states it.
...
list's `/api/search` was painted over the rendered audit. A generation counter
alone would have ignored that response while still waiting for it, which on a
...
the abort arrived, since `res.json()` then resolves normally. So: abort on
...
not a route — under the same check at no extra cost. `checks.md` records it as
`[route-supersedes]`.

...
| `provenance` | which capture a body came from. `entity/origin.py` = `body_origin` + `record`/`clear`/`page_of`; `control/resolution.py` = `origin_key()`, `own_page()` — the question that picks the gate — `cached()`, and `capture_page()`/`capture_kind()`, the two strings the browser renders next to a link; `boundary/verification.py` = `pressroom-verify-body-origin`, the read-only proof that a recorded origin reproduces the body it claims |
| `text` | bytes and markup to text. `control/decoding.py` = `decode_html()` when the declared charset lies, plus `repair_text`/`repaired` and the `REPAIRS` counter; `control/richtext.py` = `extract()`, the tag allowlist, the sanitizer, the plain-text renderer, plus `densest()`/`cut_from()`; `control/dating.py` = `iso_date()`, the single date parser; `boundary/encoding_check.py` = `pressroom-verify-encoding`, nothing repairable is stored and the two detectors agree; `boundary/container_calibration.py` = `pressroom-calibrate-containers`, the dominant container shape over every cached capture, with a page of full texts |
| `converter` | what an attachment's bytes mean: `.pdf`/`.doc` -> text (`plain_text`), or a PDF -> our HTML subset (`to_richtext`). Shells out to `pdftotext`/`antiword`; no network, no SQL. Also the two predicates that ask the same question before and after a fetch: `is_attachment_url()` on an address, `is_attachment()`/`looks_like_html()` on bytes — one magic-byte table, both directions. `boundary/calibration.py` = `pressroom-calibrate-converters`, the conversion gate's metrics over every cached attachment, with a page of full texts |
...
| `reporting` | what a run did, in one vocabulary. `control/outcome.py` = `Stats`, the seven fixed outcomes and the summary line; `control/selection.py` = which capture the archive walk did not take, drained into that summary. No table, so no entity |

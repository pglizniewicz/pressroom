...
| `scraping` | the structure a run has, independent of source. `entity/parse.py` = `Detail` and `Entry`, what a source's parser hands back; `control/discovery.py` = phase 1's three strategies plus the `capture()` the odd tails share; `control/catch_up.py` = phase 2's six strategies, each taking the parser/collector/fetcher from its caller; `control/attachment_crawl.py` = the second contract, phase 2 for `.pdf`/`.doc` rows; `control/twin.py` = filling a teaser from its twin row; `boundary/command.py` = the command line every scraper has |

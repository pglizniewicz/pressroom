#!/usr/bin/env python3
"""One-off: find the DOM container each Wayback source's article lives in.

Six parsers still extract a body by cutting the *text* of a whole page with
marker strings instead of selecting an element. Redesigning them needs a
selector, and picking one from a single capture is how you get a parser that
works on the page you looked at and silently stores navigation for the rest.
...
Usage:
  python calibrate_containers.py                    # every source with a cache
  python calibrate_containers.py --source terratec_de
  python calibrate_containers.py --source terratec --show 2
"""
...
# The row's own capture, keyed the way wayback.fetch_snapshot stores it.
_ROWS_SQL = """
...
def calibrate(source_filter=None, show=0) -> None:
    ...
    if not by_source:
        print("no cached captures for these rows yet - run "
              "backfill_body_html.py --seed-cache first")
        return

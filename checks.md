...
- [untitled] `/#r/3683` at 1280px: the row has no `title`, so the h2 reads "(bez tytułu)" rather than being empty — a heading with no text is what `repair_missing_titles.py` leaves behind on the 17 captures that carry no headline at all. The same fallback covers the result list and both neighbour links

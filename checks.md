...
- [body-plain] `/#r/6267` at 1280px: this row has no `body_html`; its body is a single `p.body--text`, the computed `white-space` is `pre-wrap`, and there is no `.body--rich`. Any `midiman_net_media_news` row will do; those teasers are confirmed unrecoverable and will stay plain
...
- [untitled] `/#r/3683` at 1280px: the row has no `title`, so the h2 reads "(bez tytułu)" rather than being empty — a heading with no text is what the corpus is left with on the 17 captures that carry no headline at all - `catch_up._fill_title` writes a title only over an empty one, and these captures have none to offer. The same fallback covers the result list and both neighbour links
...
- [badge-from-grade] `/#flags=teaser&order=date` at 1280px: every result carries the `teaser` badge, which now reads `row.grade`. Those rows have `detail_id: null` — the sentinel moved to its own column, so a badge keyed on `detail_id` would have silently gone blank on all 554 of them
...
- [panel-chains] `/` at 1280×800: the companies list fits, and a wheel over the panel still scrolls the document. `overscroll-behavior: contain` is absent — measured, Chrome ends the chain even at a scroll container with zero range, which froze the page for any wheel over the short panel

...
the table has two columns and no score:
  ...
  inferred    neither - the page had to be found by looking for the body's text
              among the captures sharing that timestamp (149 rows).
...
Usage:
  python verify_body_origin.py
  python verify_body_origin.py --source terratec_new_de
  python verify_body_origin.py --inferred-only     # just the 149 searched-for ones
"""
...
def candidate_bodies(source: str, content: bytes, page_url: str, ts: str, url: str) -> list:
    ...
    if source.startswith("terratec_new"):
        entries = scrape_terratec_new.extract_entries(content)
    ...
    if source == "midiman_de":
        return [e["body"] for e in
                scrape_midiman_de.parse_page(content, "http://www.midiman.de/", ts)]
    if source.endswith("_pressdb"):
        entries = scrape_midiman_pressdb.extract_entries(content, page_url, ts)
        mine = [e.get("body") or "" for e in entries if e.get("url") == url]
        return mine or [e.get("body") or "" for e in entries] or [
            scrape_midiman_pressdb.parse_detail(content).get("body") or ""]
    if source.endswith("_media_pr"):
        entries = scrape_midiman_media_pr.extract_entries(content, page_url, ts)
        mine = [e.get("body") or "" for e in entries if e.get("url") == url]
    ...
    if source in ("midiman_net_media_news", "midiman_com_media_news",
        ...
        out = [scrape_midiman_news.parse_detail(content).get("body") or ""]
        try:
            entries = scrape_midiman_news.extract_entries(content, page_url, ts)
    ...
    if source.startswith("terratec_press"):
        return [scrape_terratec_portal.parse_snapshot(content).get("body") or ""]
...
def attachment_verdict(body: str, body_html, content: bytes) -> str:
    ...
    if body_html:
        text, _html, _kind = attachments.to_richtext(content)
    else:
        text, _kind = attachments.plain_text(content)
    if not text:
...
def run(source: str = None, inferred_only: bool = False) -> None:
    ...
    for rid, src, url, ts, body, body_html, capture in rows:
        ...
        if not attachments.is_attachment(content) and not attachments.looks_like_html(content):
            ...
            continue
        if attachments.is_attachment(content):
            v = attachment_verdict(body, body_html, content)
        else:
            page = db.capture_page_of(capture) or capture
            try:

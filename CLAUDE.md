...
- **Every command is a console script declared in `pyproject.toml`**, never
  `python -m pressroom.<bc>.boundary.<x>`: the layer path is where the code
  lives and has no business in a command line. `pressroom-<firm>[-<generation>]`
...
cost, which measurement settled it — is in **`docs/adr/`**, one file per area,
indexed in `docs/adr/README.md`. **Read the `docs/adr/` file for an area before
changing anything in that area.**

Three neighbours own what neither of those does: **a module docstring** owns
what markup one scraper actually meets — per-scraper archaeology travels with
its parser; **`docs/layout.md`** owns the map, which file inside a component
...
and not in `docs/adr/`. A row count in any of these files is the signal —
`docs/adr/numbers.md`.
...
**Which component owns which job is `docs/layout.md`**, a lookup held to the
...
HTML, and a **parser** lifts the release out of the bytes — one from a page,
...
write path or in the scraper, and a finished fix gets deleted — git holds it.
**The same goes for the schema: there are no migrations.** Each `entity/`
layer's `SCHEMA_SQL` is the whole truth about its table; a schema change is an
edit to that DDL plus one-off SQL typed by hand.
...
2. **All three FTS triggers must exist** (`releases_ai`/`au`/`ad`), and updates
   ...
   OLD values. **`integrity-check` passing is not evidence of a healthy index** —
...
`origin.record`/`origin.clear` sit together, they are one transaction and both
...
`(parsed, confirmed)`; `confirmed=False` means archive.org failed, so nothing is
written, the item is reported `uncertain` (`?`) and stays open to a full retry;
only a confirmed absence may be `dead`. **`confirmed=True` means every capture
was tried** — no attempt cap, no fetch error, no CDX listing truncated at
`CDX_ROW_LIMIT`.
...
- **A source tag identifies a scraper — a CMS generation — not a domain, and
  ...
  (`pressemit`); the language is a property of a row, legible in its url. Tags
...
- **A per-item capture is chosen by `archive.fetch_best_matching_snapshot()`,
  ...
  **caller's** `score(content) -> int` (0 rejects), which must be the measure
  the write's own gate uses; **a tie goes to the earlier.** **`content is None`
  with a timestamp still in hand is a `stub`, not a `dead`** — captures exist
  and none scored; only a `timestamp` of None says the archive never saw the url.
...
1. a network error is not a verdict — Conventions.
...
traps: **a `.pdf`/`.doc` row must never reach an HTML parser** — BeautifulSoup
...
- **Dispatch is on magic bytes, not the extension**, and **CDX's
  ...
  each capture by how much text it yields, so a non-match is a verdict and a
  thinner early revision loses to the fuller later one.
...
- **Ask the PyCharm MCP what it says about the files you touched and their
  ...
  `Duplicated code fragment` is the finding worth the round trip — ruff has no
  copy-paste rule, and a copy's other half sits in a file the change never
  opened. Not a commit gate, and skipped where no IDE is attached.
- **`pressroom-verify-names` after any change that renames or moves a module-level
  ...
  `F821` owns undefined names — do not add that back. What it cannot see is

...
Same job as calibrate_containers.py does for DOM containers, and written for
...
numbers:
  ...
  1. **Metrics**, printed. How many documents convert, how much of the text
     ...
     thresholds in attachments.py (rotated blocks, page margins, heading
...
Usage:
  python calibrate_attachments.py                     # metrics + 12-document page
  python calibrate_attachments.py --preview all       # every document on the page
  python calibrate_attachments.py --preview 0         # metrics only
  python calibrate_attachments.py --out ~/review.html
"""
...
# into strings that exist in no version of it. See attachments.py's docstring.
...
# Same deliberate choice as attachments.py: poppler's bbox tree is XML read
# with html.parser, because this repo declares no lxml.
warnings.filterwarnings("ignore", category=XMLParsedAsHTMLWarning)


def artefacts(text: str) -> tuple:
    words = text.split()
...
def documents(conn) -> dict:
    ...
    for key, head in conn.execute(
        ...
        if attachments.is_attachment(bytes(head)) or name not in cached:
            if name not in cached or attachments.is_attachment(bytes(head)):
                cached[name] = key
...
def review(limit_preview, out_path: str) -> None:
    ...
    print("\n=== known defects, measured (thresholds live in attachments.py)")

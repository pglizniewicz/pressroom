...
scrape_midiman_pressdb.py/scrape_midiman_media_pr.py - every historical
capture of both is sampled via wayback.list_all_captures, and entries are
deduped by (title, date), keeping the longest body seen.
...
fetch_detail()/stored_detail_id() contract already proven in
scrape_midiman_news.py/scrape_maudio_news.py: a network hiccup never locks
...
treatment as scrape_midiman_pressdb.py.

Usage:
  python scrape_midiman_de.py             # everything
  python scrape_midiman_de.py --limit 3   # only sample first 3 captures per page (testing)
"""
...
def parse_inline_block(block_html: str) -> dict:
...
def parse_page(content: bytes, base_url: str, timestamp: str = None) -> list:
...
def parse_generic_page(content: bytes) -> dict:
...
def scrape(limit: int = None) -> None:
    ...
    for (title, date), e in best.items():
        ...
        parsed, confirmed = wayback.fetch_detail_snapshot(conn, session, url, parse_generic_page)

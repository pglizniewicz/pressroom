...
The second reader in this repo, next to search.py, and like it a
dependency-free one: stdlib http.server, a JSON API, and one static page of
vanilla JS. No framework, no build step, nothing added to pyproject.toml.

What it deliberately does NOT do: write. It opens the database through
db.connect_ro(), never db.connect(), so it never calls init_db(), never
migrates, never touches the FTS triggers, and an accidental write is an
OperationalError from SQLite rather than a quietly corrupted index. It binds
127.0.0.1 only - there is no auth because there is no remote listener.

Owns exactly one concern: HTTP. Every statement it runs lives in db.py; there
is no SQL in this file.

    python3 serve.py            # http://127.0.0.1:8765
    python3 serve.py --port 9000 --db /tmp/copy.db
"""
...
# no way to express traversal ('/static/../db.py' simply isn't a key here).
STATIC_FILES = {
...
def _conn(db_path):
    ...
    if getattr(_local, "conn", None) is None:
        _local.conn = db.connect_ro(db_path)
    return _local.conn
...
def _sources(params) -> list:
    """The source filter, from `source=`, `company=`, or both.

    A company IS a list of sources (companies.sources_for), so nothing here
    ...
    unknown = companies.unknown_slugs(slugs)
    if unknown:
        raise BadRequest(f"unknown company(ies): {', '.join(unknown)}; "
                         f"valid: {', '.join(companies.COMPANIES)}")
    if not slugs:
        return picked
    from_companies = companies.sources_for(slugs)
    if not picked:
...
def handle_search(conn, params) -> dict:
...
def handle_release(conn, rid: int):
    ...
    row["neighbours"] = db.neighbours(conn, rid)
    return row
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        try:
            ...
            if path == "/api/sources":
                return self._json({"sources": db.list_sources(conn)})
            if path == "/api/companies":
                ...
                return self._json({"companies": companies.roll_up(db.list_sources(conn))})
            if path == "/api/quality":
                return self._json(db.quality_counts(conn))
...
def main() -> None:
    ...
    try:
        ...
        print("Another pressroom browser is probably still running "
              "(pgrep -af 'ser[v]e.py'), or pass --port.")
        return

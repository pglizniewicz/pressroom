...
Six shapes, and they are not interchangeable:
  ...
  retry_missing   fetch the row's own capture, then ask CDX when that 404s.
                  The only shape that can turn "retried forever" into `dead`.
...
2. **`bodygate.not_shorter` always**, including under `force`. That is what
...
def pending(conn, source: str, *, force: bool = False, limit=None) -> list:
...
def _stored(conn, source: str) -> dict:
...
def from_cache(conn, source: str, parser, *, has_collector: bool = False,
    ...
    for url, detail_id in rows:
        if attachments.is_attachment_url(url):
            stats.skipped()          # a file, not a page - the attachment pass owns it
            continue
        key = captures.origin_key(conn, url, detail_id)
        own = captures.own_page(conn, url, detail_id)
        if has_collector and key and not own:
            stats.skipped()
            continue
        content = captures.cached(conn, key)
        if content is None:
            stats.skipped()
            continue
        if not attachments.looks_like_html(content):
            stats.dead()
            continue
        ...
        if not body_html:
            ...
            stats.dead()
            continue
        gate = bodygate.safe_to_write if own else bodygate.strict_same_text
        for check in (bodygate.not_shorter, gate):
            ...
            # transaction as the text. Not a clear: the key this shape reads
...
def from_listings(conn, source: str, collect, *, force: bool = False,
    ...
    Two shapes need this. midiman_de's inline releases only ever existed
    *inside* a listing page, so the scraper mints `/press/{slug}-{date}` for
    ...
    for url, e in items:
        body = e.get("body") or ""
        for check in (bodygate.not_shorter, bodygate.safe_to_write):
            ok, why = check(stored.get(url, ""), body)
            if not ok:
                held.append((url, why))
                stats.skipped()
...
def from_live(conn, source: str, fetch_body, session, *, force: bool = False,
    ...
    why this is also the offline shape for the live sources: with
...
def retry_missing(conn, source: str, parser, session, *, limit=None) -> None:
    ...
    def guarded(content, _p=parser):
        return _p(content) if attachments.looks_like_html(content) else {}

    for url, detail_id in rows:
        key = captures.capture_key(detail_id, url)
        if not key or attachments.is_attachment_url(url):
            stats.skipped()
            continue
        new_detail_id = None
        try:
            parsed = guarded(wayback.fetch_snapshot(conn, session, key))
        except Exception:
            parsed, confirmed = wayback.fetch_detail_snapshot(conn, session, url, guarded)
            if not parsed:
            ...
            key = wayback.snapshot_url(new_detail_id, url)

        body, body_html = parsed.get("body") or "", parsed.get("body_html") or ""
        if not body_html:
            stats.dead()
            continue
        ok, why = bodygate.not_shorter(stored.get(url, ""), body)
        if not ok:
...
def seed_cache(conn, source: str, session, *, force: bool = False,
    ...
    todo = [(u, d) for u, d in rows
            if captures.capture_key(d, u) and not attachments.is_attachment_url(u)]
    ...
    for url, detail_id in todo:
        ...
        try:
            ...
            stats.uncertain()
            continue
        stats.added() if attachments.looks_like_html(content) else stats.dead()
...
def catch_up(conn, source: str, *, parser=None, collect=None, fetch_body=None,
    ...
    fetcher - and this runs the shapes those make possible, in the order that
    spends the least: stored markup, then cached bytes, then the network. A
    source with no listing collector simply passes none, and the listing shape
    ...
    free shapes run and the two that fetch are skipped. `seed=True` is the
    ...
    if twins_too:
        # Last of the free shapes: a teaser whose twin row in this same source
        # already holds the real article. No capture involved, which is why it
        # lives in its own module.
        twins.fill(conn, source)
    if offline:
        return
...
# progress.py exists: the vocabulary has to be identical everywhere. A scraper
...
def no_crawl(opts: dict) -> bool:
    ...
    This is what makes `python scrape_X.py --offline` the free re-extraction of
